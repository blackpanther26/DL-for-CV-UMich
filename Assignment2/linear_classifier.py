...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    random_indices=torch.randint(low=0,high=num_train,size=(batch_size,))
...
def softmax_loss_vectorized(
    ...
    num_train=X.shape[0]
    scores=X.matmul(W)
    # keepdim=True is necessary to maintain shape (N,1) for broadcasting
    scores-=torch.max(scores,dim=1,keepdim=True).values
    exp_scores = torch.exp(scores)
    sum_exp_scores = torch.sum(exp_scores,dim=1,keepdim=True)
    probs=exp_scores/sum_exp_scores
    correct_class_prob = probs[torch.arange(num_train),y]
    loss=torch.sum(-torch.log(correct_class_prob))
    loss/=num_train
    dscores=probs.clone()
    dscores[torch.arange(num_train),y]-=1
    dW=X.t().matmul(dscores)
    dW/=num_train
    loss+=reg*torch.sum(W*W)
    dW+=2*reg*W
    #############################################################################
...
def softmax_get_search_params():
    ...
    regularization_strengths = [1,0.1,0.01,10]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
