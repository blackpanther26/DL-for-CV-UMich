...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    max_scores_per_sample = torch.max(scores,dim=1, keepdim=True).values
    shifted_scores=scores-max_scores_per_sample
    exp_score=torch.exp(shifted_scores)
    ...
    # gradient of loss wrt to scores
    # dL/df = p - y
    dscores= probs.clone()
    dscores[range(N),y]-=1
    dscores/=N
    ...
    dh1[h1<=0]=0
    grads['W1']=torch.matmul(X.T,dh1)+2*reg*W1
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores=loss_func(params,X,y=None)
...
def nn_get_search_params():
    ...
    hidden_sizes = [64,128]
    regularization_strengths = [1e-4, 1e-2, 0.1]
...
def find_best_net(
    ...
    num_classes=torch.max(y_train).item()+1
    lrs,hidden_sizes,regs,decays=get_param_set_fn()
    for lr in lrs:
        ...
        for reg in regs:
          for decay in decays:
            ...
            if val_acc>best_val_acc:
              ...
              best_net=net
              best_stat=stats
              print(f"New best val acc: {best_val_acc:.4f} (lr: {lr}, hidden: {hidden_dim}, reg: {reg})")
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
