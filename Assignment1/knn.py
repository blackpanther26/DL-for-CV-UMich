...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flat = x_test.view(num_test,-1)
    for i in range(num_train):
        diffs= x_test_flat-x_train_flat[i]
        dists[i,:]=torch.sum(diffs*diffs,dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.view(num_train,-1)
    x_test_flat = x_test.view(num_test,-1)
    #IMPLEMENTATION: ||x-y||^2 = ||x||^2+||y||^2-2<x,y>
    dists = -2*torch.mm(x_train_flat,x_test_flat.t())
    train_sq = torch.sum(x_train_flat**2,dim=1,keepdim=True)
    test_sq = torch.sum(x_test_flat**2,dim=1, keepdim=True).t()
    dists+=train_sq
    dists+=test_sq
    ##########################################################################
    #                           END OF YOUR CODE                             #
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, indices = torch.topk(dists,k=k,dim=0,largest=False) # k, num_tests
    indices=indices.t() # num_tests,k
    for j in range(num_test):
      neighbor_indices=indices[j]
      closest_y=y_train[neighbor_indices]
      votes = torch.bincount(closest_y)
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        # Current Folds - Validation set
        x_val_curr = x_train_folds[i]
        y_val_curr = y_train_folds[i]
        # Training Data - all other folds 
        x_train_curr = torch.cat(x_train_folds[:i]+x_train_folds[i+1:], dim = 0)
        y_train_curr = torch.cat(y_train_folds[:i] + y_train_folds[i+1:],dim=0)
        knn= KnnClassifier(x_train_curr,y_train_curr)
        acc = knn.check_accuracy(x_val_curr,y_val_curr)
