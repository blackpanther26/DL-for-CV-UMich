...
def create_tensor_of_pi(M: int, N: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x = torch.full((M, N), 3.14, dtype=torch.float32)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    M,N=x.shape
    col_sum=x.sum(dim=0)
    mu=(col_sum/M).reshape(1,N)
    xc=x-mu
    var=(xc*xc).sum(dim=0) /(M-1)
    sigma=var.sqrt().reshape(1,N)
    y=xc/sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    flat=torch.cat(xs)
    idx=torch.repeat_interleave(ls)
    N=ls.shape[0]
    sums=torch.zeros(N, device=flat.device,dtype=flat.dtype)
    sums.scatter_add_(0,idx,flat)
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    uniques,inverse =torch.unique(x,return_inverse=True, sorted=False)
    K=uniques.shape[0]
    N=x.shape[0]
    indices=torch.full((K,),N,dtype=torch.int64,device=x.device)
    indices.scatter_reduce_(
      dim=0,
      index=inverse,
      src=torch.arange(N,device=x.device),
      reduce='amin',
      include_self=False
    )
    ##########################################################################
