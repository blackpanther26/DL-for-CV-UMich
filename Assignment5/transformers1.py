...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
      ...
      for j in range(K):
        dot_prod=torch.dot(query[i],key[j])
        scores[j]=dot_prod/(M**0.5)
...
def scaled_dot_product_two_loop_batch(
    ...
    scores=torch.zeros((N,K,K),device=query.device,dtype=query.dtype)

    # Loop 1: Iterate over the Target Sequence Length (Queries)
    for i in range(K):
      # Loop 2: Iterate over the Source Sequence Length (Keys)
      for j in range(K):
        # Select the i-th query vector for the entire batch: Shape (N, M)
        q_vec = query[:, i, :]
        # Select the j-th key vector for the entire batch: Shape (N, M)
        k_vec = key[:, j, :]
        
        # Compute dot product across the embedding dimension M
        # (N, M) * (N, M) -> sum dim 1 -> (N,)
        dot_prod = (q_vec * k_vec).sum(dim=1)
        scores[:, i, j] = dot_prod / (M ** 0.5)

    # Apply softmax to normalize scores along the last dimension (the keys)
    # Shape becomes (N, K, K)
    weights=torch.softmax(scores,dim=-1)

    # Compute the weighted sum of values using Batch Matrix Multiplication (bmm)
    # weights (N, K, K) @ value (N, K, M) -> out (N, K, M)
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    scores=torch.bmm(query,key.transpose(1,2))/(M**.5)
    if mask is not None:
        ##########################################################################
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        c_q=(6/(dim_in+dim_q))**.5
        with torch.no_grad():
          self.q.weight.data.uniform_(-c_q,c_q)
          self.k.weight.data.uniform_(-c_q, c_q)

        c_v=(6/(dim_in+dim_v))**.5
        with torch.no_grad():
          self.v.weight.data.uniform_(-c_v,c_v)
    ...
    def forward(
        ...
        # Shape: (N, K, dim_in) -> (N, K, dim_q) for Q and K
        # Shape: (N, K, dim_in) -> (N, K, dim_v) for V
        q_proj=self.q(query)
        k_proj=self.k(key)
        v_proj=self.v(value)
        ...
        y,self.weights_softmax=scaled_dot_product_no_loop_batch(q_proj,k_proj,v_proj,mask=mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        c_linear=(6/((num_heads*dim_out)+dim_in))**0.5
        with torch.no_grad():
          self.linear.weight.data.uniform_(-c_linear,c_linear)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        # The output of this block must match the input dimension to allow
        # for residual connections later

        self.linear1=nn.Linear(inp_dim,hidden_dim_feedforward)
        self.linear2=nn.Linear(hidden_dim_feedforward,inp_dim)

        # Initialize weights for linear layer 1 (inp_dim -> hidden_dim)
        c1=(6/(inp_dim+hidden_dim_feedforward))**0.5
        with torch.no_grad():
          self.linear1.weight.data.uniform_(-c1,c1)

        c2=(6/(inp_dim+hidden_dim_feedforward))**0.5
        with torch.no_grad():
          self.linear2.weight.data.uniform_(-c2,c2)
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        # 1. MultiHead Attention 
        # The output dim must be emb_dim/num_heads so that
        # when concatenated (nums_heads*out_dim)=emb_dim

        dim_out_per_head = emb_dim // num_heads
        self.mha=MultiHeadAttention(num_heads,emb_dim,dim_out_per_head)

        # 2. Layer Normalization
        # We need 2 independent layers (one for attn bloack, one for FF block)
    ...
    def forward(self, x):
        ...
        # Sublayer 2 : Feed Forward Layer
        residual=x
        ff_output=self.ff(x)
        ff_output=self.dropout(ff_output)
        y=self.norm2(ff_output+residual)
...
def get_subsequent_mask(seq):
    ...
    N,K=seq.shape
    ones=torch.ones((K,K),device=seq.device,dtype=torch.bool)

    # Create the Upper Triangular Mask
    # torch.triu returns the upper triangular part of a matrix.
    # diagonal=1 means we exclude the main diagonal (because a word CAN see itself).
    # Result: True for future positions, False for past/present positions.
    subsequent_mask=torch.triu(ones,diagonal=1)
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.attention_self=MultiHeadAttention(num_heads,emb_dim, dim_out_per_head)
        self.attention_cross=MultiHeadAttention(num_heads,emb_dim,dim_out_per_head)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    y[:,0::2]=torch.sin(position*div_term)
    num_odd_cols=y[:,1::2].shape[1]
    y[:,1::2]=torch.cos(position*div_term[:num_odd_cols])
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        target_seq_for_mask=ans_b[:,:-1]
        mask=get_subsequent_mask(target_seq_for_mask)
