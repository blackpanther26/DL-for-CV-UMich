...
def rnn_step_backward(dnext_h, cache):
    ...
    d_activation = dnext_h*(1-next_h*next_h)
    dWx=x.t()@ d_activation
    dWh=prev_h.t()@ d_activation
    db=torch.sum(d_activation,dim=0)
    dx=d_activation@ Wx.t()
    dprev_h=d_activation@ Wh.t()
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N,T,D =x.shape
    H = h0.shape[1]
    # Initialize hidden state output tensor
    h=torch.zeros((N,T,H),device=x.device,dtype=x.dtype)
    cache=[]
    prev_h=h0

    for t in range(T):
      # select input for current time stamp
      xt=x[:,t,:]
      # prev_h becomes next_h after the funtion call
      prev_h,step_cache=rnn_step_forward(xt,prev_h,Wx,Wh,b)
      h[:,t,:]=prev_h
...
def rnn_backward(dh, cache):
    ...
    # Gradients from the future
    dprev_h=torch.zeros((N,H),device=dh.device,dtype=dh.dtype)

    for t in reversed(range(T)):
      d_current_step=dh[:,t,:]+dprev_h
      dx_t, dprev_h,dWx_t,dWh_t,db_t = rnn_step_backward(d_current_step,cache[t])
      dx[:, t, :] = dx_t
      dWx += dWx_t
      dWh += dWh_t
      db += db_t
...
class CaptioningRNN(nn.Module):
    ...
    def sample(self, images, max_length=15):
        ...
        for t in range(max_length):
          ...
          word_embed=self.embedding(prev_word)
...
def dot_product_attention(prev_h, A):
    ...
    scores=torch.bmm( # (N, 16, H) @ (N, H, 1) -> (N, 16, 1)
      A_flat.transpose(1,2), # (N, H, 16) -> transpose -> (N, 16, H)
      prev_h.unsqueeze(2), # (N, H) -> unsqueeze -> (N, H, 1)
    ).squeeze(2) # (N, 16, 1) -> squeeze (N,16)

    scores=scores/math.sqrt(H)

    # softmax over spatial locations
    attn_weights=torch.softmax(scores,dim=1) #(N,16)

    # weighted sum -> attention embedding
    attn=torch.bmm(
      A_flat, # (N, H, 16)
      attn_weights.unsqueeze(2) #(N,16,1)
    ).squeeze(2) #(N,H)
