...
class Conv(object):
    ...
    def backward(dout, cache):
        ...
        # 2. Pad Input again (needed for dw and dx calculation)
        x_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad))
        ...
        # 4. The Backward Loops
        for n in range(N):
            for f in range(F):
                # Gradient w.r.t Bias: Sum over all spatial locations
                db[f] += torch.sum(dout[n, f, :, :])
                
                for i in range(H_out):
                    for j in range(W_out):
                        # Re-calculate slice indices
                        h_start = i * stride
                        h_end = h_start + HH
                        w_start = j * stride
                        w_end = w_start + WW
                        
                        # Current upstream gradient scalar
                        # (or actually, gradient for this specific receptive field)
                        dout_val = dout[n, f, i, j]
                        
                        # Gradient w.r.t Weights (dw)
                        # Input patch * Upstream Gradient
                        mask = x_pad[n, :, h_start:h_end, w_start:w_end]
                        dw[f] += mask * dout_val
                        
                        # Gradient w.r.t Input (dx)
                        # Filter weights * Upstream Gradient
                        # We accumulate += because strides might overlap
                        dx_pad[n, :, h_start:h_end, w_start:w_end] += w[f] * dout_val
                        
        # 5. Remove Padding from dx
        # We padded (pad, pad, pad, pad), so we crop the middle
        if pad > 0:
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        pool_H=H//2
        pool_W=W//2
        flattened_dim=num_filters*pool_H*pool_W
    ...
    def __init__(self,
        ...
        for i , num_filter in enumerate(num_filters):
          ...
          if weight_scale=='kaiming':
            self.params['W'+layer_idx]=kaiming_initializer(C_in, num_filter,K=3,relu=True,device=device,dtype=dtype)
          else:
            self.params['W'+layer_idx]=torch.randn(num_filter,C_in,3,3,device=device,dtype=dtype)*weight_scale

          self.params['b'+layer_idx]=torch.zeros(num_filter,device=device,dtype=dtype)

          if self.batchnorm:
            self.params['gamma'+layer_idx]=torch.ones(num_filter,device=device,dtype=dtype)
            self.params['beta'+layer_idx]=torch.zeros(num_filter,device=device,dtype=dtype)
        ...
        flattened_dim = C_in*H*W
        last_layer_idx = str(self.num_layers)

        if weight_scale=='kaiming':
          self.params['W'+last_layer_idx]=kaiming_initializer(flattened_dim, num_classes,relu=False,device=device,dtype=dtype)
        else:
          self.params['W' + last_layer_idx] = torch.randn(flattened_dim, num_classes, device=device, dtype=dtype) * weight_scale

        self.params['b' + last_layer_idx] = torch.zeros(num_classes, device=device, dtype=dtype)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          ...
          if self.batchnorm:
            ...
            out,bn_cache=SpatialBatchNorm.forward(out,gamma,beta,self.bn_params[i])
          ...
          caches[i]=(conv_cache,bn_cache,relu_cache,pool_cache)
        ...
        dout,dw,db=Linear.backward(dout,fc_cache)
        ...
        for i in range(self.num_layers-2,-1,-1):
          ...
          grads['W' + layer_idx] = dw + 2 * self.reg * self.params['W' + layer_idx]
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    # Architecture: 3 Conv Layers -> Batch Norm
    # [32 Filters] -> Pool -> [64 Filters] -> Pool -> [128 Filters] -> Pool
    # This increasing width VGG Style works well for CIFAR-10
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache=(x,x_hat,mu,std,gamma,beta,eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x, x_hat, mu, std, gamma, beta, eps = cache
        N, D = x.shape
        ...
        dvar=(dx_hat*(x-mu)*-.5*torch.pow(std,-3)).sum(dim=0)

        dmu_part1 = (dx_hat * -1.0 / std).sum(dim=0)
        dmu_part2 = dvar * (-2.0 / N) * (x - mu).sum(dim=0)
        dmu = dmu_part1 + dmu_part2

        dx = (dx_hat / std) + (dvar * 2.0 * (x - mu) / N) + (dmu / N)
    ...
    def backward_alt(dout, cache):
        ...
        term1=N*dx_hat
        term2=dx_hat.sum(dim=0)
        term3=x_hat*(dx_hat*x_hat).sum(dim=0)

        dx=scale*(term1-term2-term3)
