...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out1,cache1=Linear.forward(X,W1,b1)
        relu1,cache_relu=ReLU.forward(out1)
        scores,cache2=Linear.forward(relu1,W2,b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        drelu1,dw2,db2=Linear.backward(dscores,cache2)
        dw2+=2*self.reg*W2
        grads['W2']=dw2
        grads['b2']=db2
        dout1=ReLU.backward(drelu1,cache_relu)
        dx,dW1,db1=Linear.backward(dout1,cache1)
        dW1+=2*self.reg*W1
        grads['W1']=dW1
        grads['b1']=db1
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, h_dim in enumerate(hidden_dims):
          layer_str=str(i+1)
          self.params['W'+layer_str]=torch.randn(curr_dim,h_dim,dtype=dtype, device=device)*weight_scale
          self.params['b'+layer_str]=torch.zeros(h_dim,dtype=dtype, device=device)
          curr_dim=h_dim
        last_layer_str=str(self.num_layers)
        self.params['W' + last_layer_str] = torch.randn(curr_dim, num_classes, dtype=dtype, device=device) * weight_scale
        self.params['b' + last_layer_str] = torch.zeros(num_classes, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        # We need a dict to store cache for the backward pass
        caches = {}
        dropout_caches = {}

        # out wilk hold the input for the next layer
        out=X

        # loop over hidden layers (1 to L-1)
        # architecture : linear --> relu --> dropout
        for i in range(self.num_layers-1):
          w=self.params['W'+str(i+1)]
          b=self.params['b'+str(i+1)]

          # Linear
          out, linear_cache = Linear.forward(out,w,b)

          # ReLU
          out,relu_cache = ReLU.forward(out)

          caches[i]=(linear_cache,relu_cache)

          if self.use_dropout:
            out,d_cache=Dropout.forward(out,self.dropout_param)
            dropout_caches[i]=d_cache
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict, num_epochs=30,batch_size=128,print_every=100,
                    device=device)
    solver.train()
    ##############################################################
    #                    END OF YOUR CODE                        #
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask=(torch.randn(x.shape, device=x.device)<(1-p))/(1-p)
            out=x*mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
